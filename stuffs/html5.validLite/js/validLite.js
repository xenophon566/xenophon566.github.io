...
jQuery.fn.validLite = function(settings) {
	...
	this.getValidata = function(e) {
		...
		elem.each(function(){
			var opt = $(this).data("validata"), pattern = eval('settings.pattern.'+opt.pattern) || /.+/,
					...
					if( settings.setting.corMsgShow && corId ) {
						if( corId == 'alert' ) eval(corId)(corMsg);
							...
							if( $("#"+corId).size() ) $("#"+corId).show();
								...
								corElem.id = corId; corElem.className = settings.setting.corElemClass; corElem.innerHTML = corMsg;
								$(this).after(corElem);
							}
						}
					}
					...
					if( settings.setting.errMsgShow && errId ) {
						if( errId == 'alert' ) eval(errId)(errMsg);
							...
							if( $("#"+errId).size() ) $("#"+errId).show();
								...
								errElem.id = errId; errElem.className = settings.setting.errElemClass; errElem.innerHTML = errMsg;
								$(this).after(errElem);
							}
						}
					}
